...
The first argument to the select function is the dataframe, the remaining arguments are the names of the variables to keep.
  ...
  + Create a new dataframe called `adls1` from `adsl` that contains: SUBJID, AGE, SEX, SMWDBL, WHOFCBL, HEIGHTBL, WEIGHTBL, TRTSDT and TRTEDT 
...
From the answer above, we should be able to see a lot of `NaN`s. NaN stands for 'Not a Number', a special type of missing number. 


2. mutate() Function
  + Create a new data frame called adsl2, which is adsl1 and an additional column: DaysBet, the number of days between TRTEDT and TRTSDT.
