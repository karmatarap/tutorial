...
```{r}
cat_vars <- adsl1 %>% select_if(is.character)
```
...
```{r}
adsl3 <- mutate(adsl2, BMIBL = WEIGHTBL / ( HEIGHTBL^2 * 10e-5))
```
  
  + Create adsl4, which combines the previous two steps into one mutate function call.
```{r}
adsl4 <- mutate(adsl1, TrtDuration = TRTEDT - TRTSDT + 1, BMIBL = WEIGHTBL / (HEIGHTBL^2  * 10e-5))
```
